...
def read_node_92():
    ...
    node_info = get_node_info(config, 'node_92')
    if not node_info:
        return {"error": "node_92 not found in config"}
    # Use BACnetReader with config values, strip subnet if present
    ...
    bacnet = BACnetReader(device_address, object_name, object_id)
    # Example: Read Room Temp (sensor_1) for node_92
    ...
    instance = int(node_info['src_name'])
    prop = 'presentValue'
    result = bacnet.read_property(target_ip, obj_type, instance, prop)
